...
results = BETSsearch(description = "i", lang = "pt", view = F)
results

data1 <- BETS::BETSget(4189)
plot.ts(window(data1,start=c(1987,1), frequency = 12))
data2 <- BETS::BETSget(432)
plot.ts(window(data2,start=c(1999,3), frequency = 365))
  
data2_new <- data2 |> 
  dplyr::mutate(
    date = as.character(date),
    date = stringr::str_sub(date, start = 1, end = 7)
  ) |> 
  dplyr::group_by(
    date, value
  ) |> 
