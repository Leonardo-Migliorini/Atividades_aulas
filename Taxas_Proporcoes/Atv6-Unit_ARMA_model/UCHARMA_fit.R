ucharma.fit <- function(y, ar = NA, ma = NA, X = NA, tau = 0.5, link = "logit") {
  if (min(y) <= 0 || max(y) >= 1) {
  ...
  z <- c()
  maxit1 <- 10000
  p <- max(ar) # valor max do vetor AR
  q <- max(ma) # valor max do vetor MA
  n <- length(y) # tamanho da série
  m <- max(p, q, na.rm = T)
  y1 <- y[(m + 1):n]
  p1 <- length(ar) # ordem do vetor AR
  q1 <- length(ma) # ordem do vetor MA
  error <- rep(0, n) # termo de erro
  eta <- rep(NA, n) # preditor linear
  ...
  if (any(linktemp == c("logit", "probit", "cloglog"))) {
    stats <- make.link(linktemp) # definindo a função de ligação
  } else {
    stop(paste(linktemp, "link not available, available links are \"logit\", ", 
               "\"probit\" and \"cloglog\""))
  }
  # definindo exprossões da função de ligação
  link <- linktemp 
  linkfun <- stats$linkfun
  linkinv <- stats$linkinv
  mu.eta <- stats$mu.eta 
  diflink <- function(t) 1 / (stats$mu.eta(stats$linkfun(t)))
  ynew <- linkfun(y) # aplicando a função de ligação na série temporal
  ynew_ar <- suppressWarnings(matrix(ynew, (n - 1), max(p, 1, na.rm = T)))

  ########################################################### 3
  
  # Definindo ordem do modelo e matriz de regressores
  if (any(is.na(ar)) == F) {
  ...
  loglik <- function(z) {
    ...
    ll <- log(log(tau) * sigma / (1 - exp((-log(mu))^sigma))) - log(y) + (sigma - 1) * log(-log(y)) +
  ...
  # estimando os valores do vetor de parâmetros do modelo
  opt <- optim(initial, loglik, escore.ucharma,
