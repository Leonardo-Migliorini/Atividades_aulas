ucharma.fit <- function(y, ar = NA, ma = NA, tau = .5, link = "logit", h = 1,
  ...
  loglik <- function(z) {
    ...
    ll <- log(log(tau) * sigma / (1 - exp((-log(mu))^sigma))) - log(x) + (sigma - 1) * log(-log(x)) +
      (-log(x))^sigma + log(tau) / (1 - exp((-log(mu))^sigma)) * (1 - exp((-log(x))^sigma))
    sum(ll)
  }
  #############################################################################
