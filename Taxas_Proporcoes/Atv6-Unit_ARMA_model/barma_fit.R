...
barma.fit<-function (y, ar = NA, ma = NA, X = NA, tau = .5,link = "logit")
{
  if (min(y) <= 0 || max(y) >= 1)
  ...
  names_par <- c("alpha",names_beta,names_phi,names_theta, "shape2")
...
set.seed(20)
for(i in 1:R){
y <- simu.barma(100,alpha=0.7, phi=0.5,theta=-.35, shape2 = 1, link = "log")
ts.plot(y)
fit<-try(barma.fit(y,ar=1,ma=1,link = "log"))
print(fit$model)
coeff[i,]<-fit$model[,1]
}
