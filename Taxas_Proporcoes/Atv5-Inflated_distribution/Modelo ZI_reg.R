library(gamlss)
#------------------------------------------------------------------------------------------
# ZIKUM in gamlss.family
#------------------------------------------------------------------------------------------
source("ZIKUMfuncs.R")
# derivatives of log.kum
kww<-expression(
  log(sigma) + log(log(1-tau)/log(1-mu^sigma)) + (sigma-1)*log(y) +
    (log(1-tau)/log(1-mu^sigma)-1)*log(1-y^sigma)
)
m1<-D(kww,"mu")
s1<-D(kww,"sigma")
ms2<-D(m1,"sigma")
#
ZIKUM<-function (mu.link = "logit", sigma.link = "log", nu.link = "logit") 
{
  mstats <- checklink("mu.link", "ZIKUM", substitute(mu.link), 
                      c("logit", "probit", "cloglog", "log", "own"))
  dstats <- checklink("sigma.link", "ZIKUM", substitute(sigma.link), 
                      c("inverse", "log", "identity"))
  vstats <- checklink("nu.link", "ZIKUM", substitute(nu.link), 
                      c("logit", "probit", "cloglog", "log", "own"))
  structure(list(family = c("ZIKUM", "Zero Inflated Kumaraswamy"), 
                 ...
                 dldm = function(y, mu, sigma) {
                   ...
                   -2 * dZIKUM(y, mu, sigma, nu, log = TRUE)
                 }, rqres = expression({
                   uval <- ifelse(y == 0, nu * runif(length(y), 0, 1), 
                                  (1 - nu) * pZIKUM(y, mu, sigma, nu))
                   rqres <- qnorm(uval)
                 }), 
...
for (i in 1:100) {
  y<-rZIKUM(n,mu_true,sigma_true,nu_true)
