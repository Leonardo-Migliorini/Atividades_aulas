...
for (i in 1:100) {
  y <- rZIUQC(n, mu_true, sigma_true, nu_true)
  fit1 <- gamlss(y ~ 1, family = "ZIUQC", trace = F)
...
for (i in 1:R) {
  ...
  fit_reg <- gamlss(
    ...
    family = ZIUQC(mu.link = "logit", sigma.link = "log", nu.link = "identity"),
    trace = F, method = RS()
  )
