...
library(gamlss)
source("OI_UQChen.R") # zero inflated unit quantile chen functions
...
OIUQC <- function(mu.link = "logit", sigma.link = "log", nu.link = "logit") {
  mstats <- checklink(
    "mu.link", "OIUQC", substitute(mu.link),
    c("logit", "probit", "cloglog", "log", "own")
  )
  dstats <- checklink(
    "sigma.link", "OIUQC", substitute(sigma.link),
    c("inverse", "log", "identity")
  )
  vstats <- checklink(
    "nu.link", "OIUQC", substitute(nu.link),
    c("logit", "probit", "cloglog", "log", "own", "identity")
  )
  structure(
    list(
      family = c("OIUQC", "One Inflated Unit Quantile Chen"),
      ...
      dldm = function(y, mu, sigma) {
        ...
        -2 * dOIUQC(y, mu, sigma, nu, log = TRUE)
      }, rqres = expression({
        uval <- ifelse(y == 1, nu * runif(length(y), 0, 1),
                       (1 - nu) * pOIUQC(y, mu, sigma, nu)
        )
        rqres <- qnorm(uval)
      }),
