...
for (i in 1:100) {
  y <- rOIUQC(n, mu_true, sigma_true, nu_true)
  fit1 <- gamlss(y ~ 1, family = "OIUQC", trace = F)
...
for (i in 1:R) {
  ...
  fit_reg <- gamlss(
    ...
    family = OIUQC(mu.link = "logit", sigma.link = "log", nu.link = "identity"),
    trace = F, method = RS()
  )
