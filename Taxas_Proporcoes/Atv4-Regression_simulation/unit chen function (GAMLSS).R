...
vscore_MLE_UMOC <- function(teta,y)
{
  ...
  # sigma
  db <- 1 / sigma - ((-log(y))^sigma) * log(-log(y)) * exp(((-log(y))^sigma)) * 
    mu + ((-log(y))^sigma) * log(-log(y)) + log(-log(y))
  
  # mu
  # mu_hat <- 1/(exp((-log(y))^sigma_hat) - 1)
  dl <- 1/mu - exp((-log(y)^sigma)) + 1
  
  
  Umu<-sum(dl); Usigma<-sum(db)
  vetor_score<-c(Umu,Usigma)
